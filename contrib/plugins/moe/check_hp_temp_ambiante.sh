...
# Remplacement de la ligne ci-dessus par 
#  CODE=$(snmpwalk -v 2c -c $COMMUNITY $HOST .1.3.6.1.4.1.232.6.2.6.8.1.3 | cut -f2 -d '( ' | cut -f1 -d ')' | awk NR==$AMOUNT)
...
if [ $CODE -eq 11 ]; then # Modification du script originel pour prendre en compte la température ambiante
 if [ $AMOUNT -gt 1 ];then 
  if [ $TEMP -gt $LIMIT ];then
    WARNING=1
    printf "WARNING: $DESCR depasse la limite de  $LIMIT C avec $TEMP C "
   else
    printf "OK: $DESCR est de $TEMP C "
   fi
 fi
fi
 AMOUNT=$[$AMOUNT-1]
