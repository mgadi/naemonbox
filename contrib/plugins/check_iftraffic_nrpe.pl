...
#my $out_traffic_absolut = sprintf( "%.0d", $last_out_bytes * 1024 );
 ...
 my $in_bits = $in_traffic * 1024 * 8 ;
 my $out_bits = $out_traffic * 1024 * 8 ;
...
my $output = "";
$output .=
    "$iface_descr Average Traffic: $in_traffic "
  . $in_prefix . "o/s ("
  . $in_usage
  . "%) in, $out_traffic "
  . $out_prefix . "o/s ("
...
. " inBandwidth=" . $in_bits . "bits/s " . "outBandwidth=" . $out_bits . "bits/s\n";

}

if (($in_usage>$warn_usage) && (($in_usage<$crit_usage)) or ($out_usage > $warn_usage ) && $exit_status eq "OK" )
{
$exit_status = "WARNING";
$output .=
"|inUsage=$in_usage%;$warn_usage;$crit_usage outUsage=$out_usage%;$warn_usage;$crit_usage"
